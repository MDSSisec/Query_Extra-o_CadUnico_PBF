...
/* 5) PBF por CPF: existe família do CPF na folha?
   ...
   Mas se NENHUM registro do CPF tem CO_EST_CADASTRAL_MEMB = 3 → FL_PBF = 0
  ...
  SELECT
      ...
      CASE
        ...
        /* Regra nova:
           ...
           Se NÃO existe nenhuma pessoa com estado 3 → não pode ter PBF

        */

        WHEN MAX(CASE WHEN P.CO_EST_CADASTRAL_MEMB = 3 THEN 1 ELSE 0 END) = 0

        THEN 0
 
        /* Caso contrário segue regra normal da folha */

        ELSE

          MAX(CASE WHEN fa.CO_FAMILIAR IS NOT NULL THEN 1 ELSE 0 END)
